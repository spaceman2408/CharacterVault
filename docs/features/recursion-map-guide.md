...
## The three flags, in plain terms
...
**A chain explodes into everything.** Inspect the entry where the explosion starts and look at its "Unlocks" list. Usually one broad key (a name, a place) is matching half the book. Either tighten that target's keys, or set **Prevent further recursion** on the entry that fans out.

**Two entries keep re-triggering each other.** A mentions B's key, B mentions A's key. Flags can't break the loop by themselves; the clean fix is removing one direction's key from the other entry's content. Use the map to find both arrows, then **Open in editor** on either side.

**An entry never fires recursively.** Click it and check "Unlocked by". Empty? Either nothing mentions its keys (rename the keys or mention them where you want), or **Non-recursable** is on, which the panel will say directly.

**Deep lore fires too early.** Set **Delay until recursion** so it stays quiet on the first chat scan and only joins once the topic is already active.
