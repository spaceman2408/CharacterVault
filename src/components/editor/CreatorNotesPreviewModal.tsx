  ...
  return createPortal(
    <div className="fixed inset-0 z-120 flex items-center justify-center bg-black/65 p-4 backdrop-blur-sm">
      <div
        ...
        aria-hidden="true"
      />
      <div className="relative z-10 flex h-[min(85vh,900px)] w-[min(1100px,100%)] flex-col overflow-hidden rounded-2xl border border-vault-200 bg-white shadow-2xl dark:border-vault-700 dark:bg-vault-900">
        ...
        <div className="flex-1 bg-vault-950 p-4">
          <CreatorNotesPreviewPane
            content={content}
            className="h-full"
            frameClassName="block h-full w-full rounded-xl border border-vault-700 bg-vault-950"
            emptyClassName="flex h-full items-center justify-center rounded-xl border border-dashed border-vault-600 bg-vault-950/70 px-5 py-6 text-sm text-vault-300"
          />
        </div>
      </div>
    </div>,
